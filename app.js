...
function startRealtime() {
  const watch = (name) => {
    const q = firebaseApi.collection(firebaseApi.db, name);
...
async function saveCollectionItem(collectionName, item) {
  ...
  await firebaseApi.setDoc(firebaseApi.doc(firebaseApi.db, collectionName, item.id), item, { merge: true });
}
...
async function deleteCollectionItem(collectionName, id) {
  ...
  await firebaseApi.deleteDoc(firebaseApi.doc(firebaseApi.db, collectionName, id));
}
